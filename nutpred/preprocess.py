...
def extract_topk_from_ingnut(ingnut_df: pd.DataFrame, k: int = None) -> Tuple[pd.DataFrame, List[str]]:
    ...
    ingredient_col = "Snack_Top135_Ing"
...
def calculate_mapped_ratio(row):
    ...
    mapped_count = len(row.get('mapped_list', []))
    return mapped_count / max(raw_count, 1)
        
def process_ingredients(food_df: pd.DataFrame, ingnut_df: pd.DataFrame, k: int = 133) -> pd.DataFrame:
    """
    ...
    # Step 2: Filter to top-k (now with resolved butter variants)
    food_df = filter_ingredients_to_topk(food_df, top_list)
    
    # Step 3: Create binary/score features from resolved ingredients
    binary_df, score_df = build_binary_and_scores(food_df, top_list)
    
    # Step 4: Join features to food_df dataframe
