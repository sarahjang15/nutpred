...
def predict_ingnut_weights_and_targets(food_df: pd.DataFrame, ingnut_df: pd.DataFrame, nut8_cols: List[str], 
                                     ...
                                     scale: str = "std", test_indices: List[int] = None, top_list: List[str] = None) -> Tuple[pd.DataFrame, np.ndarray, List[str], List[int]]:
    """
    ...
    try:
        ...
        logger.info("Mapping ingredients to ingredients...")
        mapped_tokens = []
        progress_step = max(1, len(food_df) // 100)  # Progress every 1%
        
        
        for j, (_, row) in enumerate(food_df.iterrows()):
            if j % progress_step == 0:
                logger.info(f"Mapping progress: {j}/{len(food_df)} samples processed, {len(mapped_tokens)} tokens mapped")
            ...
            row_tokens = []
            for token in tokens:
                # Try to resolve the variant (this handles both direct matches and rule-based resolution)
        ...
        if test_indices is not None:
            # Use all samples except test samples for training
            train_indices = [i for i in range(len(food_df)) if i not in test_indices]
            logger.info(f"Using train/test split: {len(train_indices)} training samples, {len(test_indices)} test samples")
            
            # Run optimization on training samples only
            train_mapped_tokens = [mapped_tokens[i] for i in train_indices]
            train_Y_mat = Y_mat[train_indices]
            
            logger.info("Running optimization on training samples only...")
            train_preds_w, failed = _run_optimization(
                train_mapped_tokens, X_all, train_Y_mat, constraint=constraint, ridge=ridge, 
                robust=robust, solver_name=solver_name, scale=scale, top_list=top_list
            )
            
            # Create full prediction matrix
            preds_w = np.zeros((len(food_df), len(ingnut_df)))
            preds_w[train_indices] = train_preds_w
            
            # For test samples, use the same optimization but with training-learned parameters
            # This is a simplified approach - in practice, you might want to use cross-validation
            logger.info("Generating predictions for test samples using training-learned approach...")
            for i, test_idx in enumerate(test_indices):
                if test_idx < len(mapped_tokens):
                    test_tokens = mapped_tokens[test_idx]
                    if len(test_tokens) > 0:
                        # Use uniform weights for test samples (or could use average of training weights)
                        test_weights = np.zeros(len(ingnut_df))
                        test_weights[test_tokens] = 1.0 / len(test_tokens)
                        preds_w[test_idx] = test_weights
        else:
            # Run optimization on all samples (no train/test split)
        ...
        for j, c in enumerate(nut8_cols):
            food_df[f"opt_{c}"] = Y_pred[:, j]
            logger.info(f"Added prediction column: opt_{c}")
        ...
        # Create mapping for target nutrients
        target_ingnut_mapping = {
            'Calcium(mg)': 'Calcium, Ca',
            'Fiber(g)': 'Fiber, total dietary', 
            'Iron(mg)': 'Iron, Fe'
        }
        
        for target, ingnut_col in target_ingnut_mapping.items():
            if ingnut_col in ingnut_df.columns:
                ...
                food_df[f"opt_{target}"] = target_pred
...
def _run_optimization(mapped_tokens: List[List[int]], X_all: np.ndarray, Y_mat: np.ndarray,
    ...
    logger.info(f"Scaling weights calculated. Mean weight: {np.mean(W_vec):.4f}, std: {np.std(W_vec):.4f}")
...
# Evaluate ing_pred
# ---------------------------------------------------------------------
def eval_ing_pred(df: pd.DataFrame, failed_indices: list = None, test_indices: list = None, nut8_cols: list = None) -> pd.DataFrame:
    ...
    for nutrient in all_nutrients:
        pred_col = f"opt_{nutrient}"
        if nutrient in df.columns and pred_col in df.columns:
            y = df[nutrient].to_numpy(float)
            yhat = df[pred_col].to_numpy(float)

            eval_mask_all = (df["failed"] == 0) & ~(pd.isna(y) | pd.isna(yhat))
            eval_masks =[eval_mask_all]
            sample_types = ["all"]

            # Calculate metrics for test samples only
            if test_indices is not None:
                # Create mask for test samples that also pass strict filter
                test_mask = df.index.isin(test_indices)
                eval_mask_test = test_mask & (df["failed"] == 0) & ~(pd.isna(y) | pd.isna(yhat))
                eval_masks.append(eval_mask_test)
                sample_types.append("test")

                for eval_mask, sample_type in zip(eval_masks, sample_types):
                    # Use these samples for evaluation
                    y_eval = y[eval_mask]
                    ...
                    #logger.info(f"  {nutrient} ({sample_type}): R²={r2_eval:.4f}, RMSE={rmse_eval:.4f}, MAE={mae_eval:.4f}, SMAPE={s_eval:.4f}%")
                    
                    rows.append({"Nutrient": nutrient, "FeatureSet": "ing_pred", "Model": "Optimization", "Filter": "strict", 
                    "SampleType": sample_type, "R2": r2_eval, "RMSE": rmse_eval, "MAE": mae_eval, "MAPE": mape_eval, "SMAPE": s_eval})
            else:
                logger.warning(f"No valid data for {nutrient} (test samples)")
